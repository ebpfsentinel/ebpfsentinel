...
impl LbAppService {
    ...
    fn sync_ebpf_maps(&mut self) {
        ...
        // Push Maglev lookup rings for consistent-hash services. The
        // table is generated by the pure domain engine and keyed by the
        // same per-service index the eBPF data plane derives.
        let maglev_services: Vec<(String, u32)> = self
            .engine
            .services()
            .iter()
            .filter(|s| s.enabled && s.algorithm == LbAlgorithm::Maglev)
            .map(|s| {
                let key = service_to_ebpf_key(s);
                (s.id.to_string(), lb_service_index(key.protocol, key.port))
            })
            .collect();

        for (service_id, svc_index) in maglev_services {
            match self.engine.maglev_table(&service_id) {
                Some(table) => {
                    if let Err(e) = port.sync_maglev_table(svc_index, table) {
                        tracing::warn!(
                            service = %service_id,
                            "failed to sync Maglev table to eBPF: {e}"
                        );
                    }
                }
                ...
                None => {
                    if let Err(e) = port.remove_maglev_table(svc_index) {
                        tracing::warn!(
                            service = %service_id,
                            "failed to remove stale Maglev table: {e}"
                        );
                    }
                }
            }
