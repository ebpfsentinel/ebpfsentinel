...
use ebpf_common::{
    conntrack::{
        ConnKey, ConnValue, CT_FLAG_NAT_DST, CT_MAX_ENTRIES_V4, CT_STATE_NEW, normalize_key_v4,
    },
    nat::{
        MAX_NAT_RULES, NAT_MATCH_DST_IP, NAT_MATCH_DST_PORT, NAT_MATCH_PROTO, NAT_MATCH_SRC_IP,
        NAT_METRIC_COUNT, NAT_METRIC_DNAT_APPLIED, NAT_METRIC_ERRORS, NAT_TYPE_DNAT,
        NAT_TYPE_ONETOONE, NAT_TYPE_REDIRECT, NatRuleEntry,
    },
};
use network_types::{
    eth::EthHdr,
...
/// Check if a NAT rule matches the packet.
#[inline(always)]
fn match_nat_rule(
    rule: &NatRuleEntry,
    src_ip: u32,
