...
use aya_ebpf::{
    bindings::TC_ACT_OK,
    helpers::{bpf_ktime_get_boot_ns, bpf_l3_csum_replace, bpf_l4_csum_replace, bpf_skb_store_bytes},
...
use ebpf_common::{
    conntrack::{
        ConnKey, ConnValue, CT_FLAG_NAT_SRC, CT_MAX_ENTRIES_V4, normalize_key_v4,
    },
    nat::{
        MAX_NAT_RULES, NAT_MATCH_DST_IP, NAT_MATCH_PROTO, NAT_MATCH_SRC_IP, NAT_METRIC_COUNT,
        NAT_METRIC_ERRORS, NAT_METRIC_MASQ_APPLIED, NAT_METRIC_PORT_ALLOC_FAIL,
        NAT_METRIC_SNAT_APPLIED, NAT_TYPE_MASQUERADE, NAT_TYPE_SNAT, NatPortAllocKey,
        NatPortAllocValue, NatRuleEntry, MAX_NAT_PORT_ALLOC,
    },
};
use network_types::{
    eth::EthHdr,
...
/// Allocate a port from the rule's port range using the port allocation table.
/// Uses a simple hash-based allocation: hash(orig_addr, orig_port) into range.
#[inline(always)]
fn allocate_port(orig_addr: u32, orig_port: u16, range_start: u16, range_end: u16) -> u16 {
    ...
    // Simple hash: mix address and port for distribution across the range.
...
/// Check if a SNAT rule matches the packet.
#[inline(always)]
