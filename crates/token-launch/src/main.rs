...
fn collect_module_btf_fds() -> Vec<(String, RawFd)> {
...
fn delegate_over_fd(fs: RawFd) -> bool {
...
fn send_msg_fds(sock: RawFd, payload: &[u8], fds: &[RawFd]) -> bool {
    let mut iov = libc::iovec {
...
fn recv_msg_fds(sock: RawFd, buf: &mut [u8], max_fds: usize) -> (usize, Vec<RawFd>) {
    let mut iov = libc::iovec {
