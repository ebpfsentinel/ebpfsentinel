...
load '../integration/lib/pktgen_helpers'
...
setup_file() {
    ...
    if [ "${EBPF_2VM_MODE:-false}" != "true" ]; then
        skip "perf/04 requires EBPF_2VM_MODE=true (pktgen on attacker NIC)"
    fi
...
@test "pktgen baseline — measures achievable pps and idle CPU" {
    ...
    # unreachable (env-limited, like perf/01): accept any non-trivial rate
